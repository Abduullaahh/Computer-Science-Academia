...
int main()
{
	char* Name; double Dail_Ups; string name;
	int Hours, MBs, opt = 1, Bill1 = 0, Bill2 = 0, Bill3 = 0;
	Customer* Customer_Plan[3];
	while (opt != 0)
	{
		system("cls");
		Name = new char[20];
		cout << "Enter your name : "; cin >> name; int i = 0; for (; name[i] != '\0'; i++) { Name[i] = name[i]; } Name[i] = '\0';
		...
		{
			cout << "Mr. " << Customer_Plan[1]->get_Name() << ", you should get Hacker plan because it will cost you least, that is " << Bill2 << "$";
		}
		else if (Bill3 < Bill1 && Bill3 < Bill2)
		{
			cout << "Mr. " << Customer_Plan[2]->get_Name() << ", you should get ChatRoom plan because it will cost you least, that is " << Bill3 << "$";
		}
		cout << endl;
		...
		if (opt == 0)
		{
			/*for (int i = 0; i < 3; i++)
			{
				delete[]Customer_Plan[i]; Customer_Plan[i] = NULL;
			}
			delete[]Customer_Plan;*/
