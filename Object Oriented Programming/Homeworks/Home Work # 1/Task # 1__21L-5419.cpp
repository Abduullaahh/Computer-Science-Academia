...
int Menu()
{
	...
	cout << "PRESS 4 : Increments / Decrements in fractions (post OR pre)\n";
...
int main()
{
	...
	for (int opt = 0; opt != -1;)
	{
		...
		else if (opt == 2)
		{
			Fraction Sum;
			Sum = F1 + F2;
			cout << "Sum : " << Sum;/*cout << "Sum : " << F1 + F2;*/
		}
		else if (opt == 3)
		{
			Fraction Product;
			Product = F1 * F2;
			cout << "Product : " << Product;/*cout << "Sum : " << F1 * F2;*/
		}
		else if (opt == 4)
		{
			s = 0;
		...
		{
			cout << "Total fractions created : " << Fraction::getCount();
		}
		else if (opt == 9)
		{
			s = 0;
